  ...
  const handleGeneratePlan = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/preparation-schedule",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            skills_to_develop: skillsToAdd
              .split(",")
              .filter((s) => s.trim())
              .map((s) => s.trim()),
            has_resume: !!userProfile.resume_content,
            has_cover_letter: !!userProfile.cover_letter_content,
          }),
        }
      );
